...
class BaseController:
    ...
    def train(self,
        ...
        for epoch in range(configs.agent.num_epochs):
            ...
            layers = [self.search_space[i] for i in layer_IDs]
    ...
    def eval(self, train_data, test_data, configs):
        with torch.no_grad():
            ...
            layer_IDs, _ = self.controller.sample(sample_best=True)
            layers = [self.search_space[i] for i in layer_IDs]
...
class _Controller(nn.Module):
    def __init__(self, search_size, num_outputs, hidden_size=128, device=torch.device('cpu')):
        ...
        self.encoder = torch.nn.Embedding(search_size, hidden_size)
        self.cell = nn.LSTMCell(input_size=hidden_size, hidden_size=hidden_size)
        self.decoder = nn.ModuleList([nn.Linear(hidden_size, search_size) for _ in range(num_outputs)])
